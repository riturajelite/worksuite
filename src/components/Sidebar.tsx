...
export default function Sidebar({ currentTab, onSelectTab, isMobileOpen, onClose, userRole, onLogout }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside 
        ...
        className={`fixed inset-y-0 left-0 z-50 lg:relative lg:z-30 w-64 bg-[#0b0f19] text-slate-200 flex flex-col border-r border-slate-800/60 h-screen overflow-hidden select-none transition-transform duration-300 ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1.5">
            <button className="flex items-center gap-1 font-bold text-base text-white tracking-wide hover:text-indigo-400 transition-colors">
              <span>Worksuite</span>
              <ChevronDown className="h-4 w-4 text-slate-400 mt-0.5" />
            </button>
            <div className="flex items-center gap-2 px-0.5">
              <div className="w-2.5 h-2.5 bg-[#22c55e] rounded-full border border-emerald-400 animate-pulse" />
              <span className="text-xs font-semibold text-slate-300">
                Eldora Mann MD
              </span>
            </div>
          </div>
          {/* Branded Logo Square on top right */}
          <div className="w-9 h-9 bg-[#f59e0b] rounded-lg flex items-center justify-center text-white font-black text-xl shadow-md select-none shrink-0">
            W
          </div>
        </div>
      </div>
      ...
      {/* Quick Search */}
      <div className="px-3 py-2 border-b border-slate-800/40 bg-slate-950/20 shrink-0">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-0.5 scrollbar-thin scrollbar-thumb-slate-800/60 scrollbar-track-transparent">
        {searchTerm ? (
          ...
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-500 tracking-wider uppercase px-3 mb-2">Search Results ({filteredItems.length})</p>
            ...
            {sidebarItems.map(item => {
              ...
              return (
                <div key={item.id} className="space-y-0.5">
                  ...
                  {/* Child list items */}
                  {item.isExpandable && isExpanded && item.items && (
                    <div className="pl-4 space-y-0.5 border-l border-slate-800/80 ml-5 mt-0.5 mb-1.5">
      ...
      {/* Sidebar Footer Zone */}
      <div className="p-3 border-t border-slate-800 bg-[#070b13] flex items-center justify-between shrink-0 select-none">
        <button 
        ...
        <button 
          ...
          onClick={() => alert('Worksuite Mobile App link active! Navigate to the QR Code module to scan.')}
          className="flex items-center gap-1.5 bg-sky-950 hover:bg-sky-900 border border-sky-800/80 text-sky-200 text-[11px] font-bold px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer shadow-xs"
        >
