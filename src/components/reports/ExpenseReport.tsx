...
import { 
  ...
  Printer, Copy, FileDown, Receipt, ShieldAlert, HeartPulse, Sparkles
} from 'lucide-react';
import { MOCK_EXPENSE_REPORTS, EMPLOYEES, ExpenseReportItem } from './mockReportsData';
...
export default function ExpenseReport() {
  ...
  const [sortField, setSortField] = useState<keyof ExpenseReportItem>('date');
  ...
  }, [selectedEmployee, selectedStatus, searchTerm]);
  ...
  const handleExport = (type: string) => {
    triggerToast(`Exporting Expense Report as ${type.toUpperCase()}...`);
  };

  const handleSort = (field: keyof ExpenseReportItem) => {
    if (sortField === field) {
  ...
  // Filter Data
  const filteredData = MOCK_EXPENSE_REPORTS.filter(item => {
  ...
  // Metrics
  const totalApproved = filteredData.filter(i => i.status === 'Approved').reduce((s, i) => s + i.amount, 0);
  const totalPending = filteredData.filter(i => i.status === 'Pending').reduce((s, i) => s + i.amount, 0);
  const totalRejected = filteredData.filter(i => i.status === 'Rejected').reduce((s, i) => s + i.amount, 0);
  const totalClaimed = totalApproved + totalPending + totalRejected;

  // Chart Formatting: Allocations per category
  ...
  const chartData = Object.entries(categorySummary).map(([name, value], index) => ({
    name,
    value,
  ...
  return (
    <div className="space-y-4 animate-fade-in text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-black tracking-tight text-slate-900">Expense Report</h1>
          <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Reports &gt; Expense Report</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-xs grid grid-cols-1 md:grid-cols-3 gap-3 text-xs font-semibold">
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Approval Status</label>
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            ...
            <input 
              type="text"
              placeholder="Search items or merchant..."
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="bg-white p-4.5 rounded-xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Total Claimed Outlays</p>
            <h3 className="text-xl font-black text-slate-900">${totalClaimed.toLocaleString()}</h3>
          </div>
        ...
        <div className="bg-white p-4.5 rounded-xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Approved Expenses</p>
            <h3 className="text-xl font-black text-emerald-600">${totalApproved.toLocaleString()}</h3>
          </div>
        ...
        <div className="bg-white p-4.5 rounded-xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Pending Approvals</p>
            <h3 className="text-xl font-black text-amber-600">${totalPending.toLocaleString()}</h3>
          </div>
        ...
        <div className="bg-white p-4.5 rounded-xl border border-slate-200/80 shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Rejected Audits</p>
            <h3 className="text-xl font-black text-rose-600">${totalRejected.toLocaleString()}</h3>
          </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
        <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider border-b border-slate-100 pb-2 mb-4">
          Expense Share allocation by Category
        </h3>
        <div className="h-56">
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <RechartsPieChart>
                ...
                <RechartsTooltip formatter={(value) => [`$${value.toLocaleString()}`, 'Total Spent']} />
                <Legend verticalAlign="bottom" height={36} iconSize={8} iconType="circle" wrapperStyle={{ fontSize: '9px', fontWeight: 'bold' }} />
              </RechartsPieChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-full flex items-center justify-center text-xs text-slate-400 italic">No corresponding claims logged</div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="text-slate-900 font-bold text-xs uppercase tracking-wider">Expense Claims Ledger</div>
          <div className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">
            Showing <span className="text-slate-700">{Math.min(startIndex + 1, totalItems)}-{Math.min(startIndex + itemsPerPage, totalItems)}</span> of <span className="text-indigo-600">{totalItems}</span> Claims
          </div>
        </div>
        ...
        {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-400 text-[9px] uppercase font-extrabold tracking-wider select-none">
                  ...
                  <th onClick={() => handleSort('item')} className="px-5 py-3.5 cursor-pointer hover:bg-slate-100 transition-colors">Purchased Item</th>
                  <th onClick={() => handleSort('merchant')} className="px-5 py-3.5 cursor-pointer hover:bg-slate-100 transition-colors">Merchant / Vendor</th>
                  <th onClick={() => handleSort('employee')} className="px-5 py-3.5 cursor-pointer hover:bg-slate-100 transition-colors">Employee</th>
                  <th onClick={() => handleSort('date')} className="px-5 py-3.5 cursor-pointer hover:bg-slate-100 transition-colors">Purchase Date</th>
              ...
              <tbody className="divide-y divide-slate-100 text-xs text-slate-600">
                {paginatedData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3.5 font-bold text-slate-950">{item.id}</td>
                    <td className="px-5 py-3.5 font-bold text-indigo-600">{item.item}</td>
                    <td className="px-5 py-3.5 font-semibold text-slate-500">{item.merchant}</td>
                    ...
                    <td className="px-5 py-3.5 font-semibold text-slate-400">{item.category}</td>
                    <td className="px-5 py-3.5 font-mono text-[11px] font-black text-right text-slate-900">${item.amount.toLocaleString()}</td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-block text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase ${
                        ...
                        item.status === 'Pending' ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                      }`}>
                        {item.status}
                      </span>
                    </td>
          ...
          <div className="p-12 text-center text-xs text-slate-400 italic">No corresponding claims loaded matching exact criteria.</div>
        )}
